<!-- 此文件由机器翻译自 data_access.md -->

# 数据访问和检索

本参考内容涵盖了 OMERO 分层数据结构的导航和对象的检索。
...
- `"Well"`
- `"Roi"`
- `"Annotation"`（以及特定类型：`"TagAnnotation"`、`"FileAnnotation"`等）
- `"Experimenter"`
- `"ExperimenterGroup"`
- `"Fileset"`
...
2. **限制结果**：对于大型数据集使用 `limit` 和 `offset`
3. **早期过滤**：应用过滤器以减少数据传输
4. **Check for None**：使用前始终检查`getObject()`是否返回None
5. **高效遍历**：使用`listChildren()`而不是单独查询
