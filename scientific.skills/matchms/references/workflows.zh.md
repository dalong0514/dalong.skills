...
本文档提供了使用 matchms 的常见质谱分析工作流程的详细示例。
...
1. **始终处理查询和引用**：应用相同的过滤器以确保一致的比较
2. **保存中间结果**：使用pickle格式快速重新加载处理后的光谱
3. **监控内存使用情况**：对大文件使用生成器而不是一次性加载所有文件
4. **验证数据质量**：在相似性计算之前应用质量过滤器
5. **选择适当的相似性度量**：CosineGreedy 用于速度，ModifiedCosine 用于相关化合物
6. **组合多个指标**：使用多个相似性分数进行稳健识别
7. **首先按前体质量过滤**：显着加快大型图书馆搜索速度
8. **记录您的流程**：保存处理参数以实现可重复性

## 更多资源

- 匹配文档：https://matchms.readthedocs.io
